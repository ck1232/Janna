<%@ taglib uri="http://tiles.apache.org/tags-tiles" prefix="tiles"%>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form"%>
<form:form id="createUserForm" method="get" action="/JJ/createUser"></form:form>

<div class="margin">
	<div class="btn-grp">
		<button class="btn btn-default btn-primary pull-right" type="submit" form="datatableForm" formaction="/JJ/deleteUser"><i class="fa fa-user-times"></i> Delete User</button>
		<button class="btn btn-default btn-primary pull-right" type="submit" form="createUserForm"><i class="fa fa-user-plus"></i> Add User</button>
	</div>
</div>